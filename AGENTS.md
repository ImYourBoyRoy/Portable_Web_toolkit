...
1. `Web_Toolkit/README.md` — phases, modules, site-profile model
2. `Web_Toolkit/OPERATIONS.md` — canonical operator sequence
3. `Web_Toolkit/RUNBOOKS.md` — task-specific flows
4. `Web_Toolkit/ARCHITECTURE.md` — module map
5. `Web_Toolkit/CHECKLIST.md` — pre-ship gates
6. `Web_Toolkit/MEMORY.md` — recent validated facts only
7. `docs/cursor-setup.md` — install the Cursor skill

**Do not** use `Web_Toolkit/AGENT.md` — it is a deprecated generic template. This file replaces it for toolkit work.
...
1. `Setup_agent_environment` / `project-init audit` on uncertain roots
2. `init_site_profile` or load existing profile
3. `toolkit-report` when phase is unclear
4. Build path: `npm run check` → `npm run build`
5. `discovery-doctor` on `./dist` before deploy; again on live URL after prod
6. Staging deploy + smoke (`site-quality-smoke`, `browser-diagnostics`, `integration-doctor`) before production
7. `cf-agent` audits (permissions, DNS, rules, workers) — dry-run hardening first
8. Before sharing toolkit: `toolkit-purge --apply`, `privacy-check`, `toolkit-verify`, export
...
Skill name: **portable-web-toolkit**
